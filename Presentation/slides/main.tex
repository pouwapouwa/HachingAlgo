...
$$P_{Success}=1- (\frac{N - 1}{N}) \times (\frac{N - 2}{N}) \times \cdots \times (\frac{N - K + 1}{N}) $$
  \vfill
\end{frame}
...
$$P_{Success} > 50\% \mbox{ pour } 2^{56} \mbox{ calculs de condensés.}$$
...
\begin{itemize}
\item 5 colonnes indexées horizontalement par x.
\item 5 lignes indexées verticalement par y.
\item w allées indexées en profondeur par z.
\end{itemize}
}
...
\begin{frame}
  \frametitle{Paramètres Keccak pour SHA-3}
  \vfill
\textbf{SHA-3 implémente \textsc{Keccak}$\lbrack 1600, 24 \rbrack$}
\begin{itemize}
\item{$n_r=24$}
\item{$b = 1600 = 2^6\times 25$}
\item{$l = 6$}
\end{itemize}
  \vfill
\end{frame}
...
\begin{frame}
  \frametitle{Keccak: vision d'ensemble}
  \vfill
  \begin{itemize}
  \item{nombre d'itérations: $12+2\times l$}
  \item{Une itération \textsc{Rnd} est constituée de 5 fonctions:
