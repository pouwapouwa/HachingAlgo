...
\begin{frame}
  ...
  \vfill
  \begin{itemize}
    \item $\overline{S}$: partie externe de l'état, les premier $r$ bits de l'état.
  \item $\widehat{S}$: partie interne de l'état, de taille $b-r$ bits.
  \end{itemize}
 \begin{figure}[H]
        \centering
        \begin{tikzpicture}[scale=0.7]
        ...
        \node [align=center] at (0,1){\textbf{$\overline{S}$}};
        \node [align=center]   at (0,-2){\textbf{$\widehat{S}$}};
...
\caption{\label{fig:sponge}Construction de fonctions en éponge $Z = \textsc{sponge}\lbrack f , pad, r\rbrack (M, l)$}
\end{figure}
  \vfill
\end{frame}

\begin{frame}
  \frametitle{La permutation Keccak}
  \vfill
